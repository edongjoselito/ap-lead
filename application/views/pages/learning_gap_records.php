            ...
            <table class="table lg-records-table mb-0">
                ...
                <tbody class="record-group">
                    ...
                    <tr class="record-detail-row"><td colspan="6"><details><summary>View competencies &amp; intervention details<span class="sr-only"> for <?= !$is_school ? html_escape($row->schoolName) . ', ' : ''; ?><?= html_escape($row->grade_level . ', ' . $row->learning_area . ', ' . $row->term); ?></span></summary><div class="record-details-grid">
                        ...
                        <?php foreach (array('intervention_action' => 'Intervention / Action', 'remarks' => 'Remarks') as $field => $label): ?><div><h3><?= $label; ?></h3><p><?= trim((string) $row->$field) !== '' ? nl2br(html_escape($row->$field)) : '—'; ?></p></div><?php endforeach; ?>
                    </div></details>
                    <?php if ($is_school): ?><div class="record-actions"><a class="btn btn-sm btn-outline-primary" href="<?= base_url('Pages/learning_gap_entry?edit=' . (int) $row->id); ?>"><i class="mdi mdi-pencil" aria-hidden="true"></i> Edit record</a><?= form_open('Pages/learning_gap_delete', array('style' => 'display:inline;', 'onsubmit' => "return confirm('Remove this record?');")); ?><input type="hidden" name="id" value="<?= (int) $row->id; ?>"><button class="btn btn-sm btn-outline-danger" type="submit"><i class="mdi mdi-delete-outline" aria-hidden="true"></i> Delete</button><?= form_close(); ?></div><?php endif; ?>
                    </td></tr>
                </tbody>
                <?php endforeach; ?>
            </table>
